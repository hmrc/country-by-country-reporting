  ...
  "Email Service" - {
    ...
    "sendEmail" - {
      ...
      "must submit to the email connector once for Agent only when Agent and client both have 1 set of valid details provided for failed file upload" in {
        ...
        val result =
          emailService.sendEmail(subscriptionId, submissionTime, messageRefId, Some(agentSingleContactDetails), isUploadSuccessful = false, reportType)
      ...
      "must submit to the email connector 3 times when Agent has 1 set and client has 2 sets of valid details provided for successful file upload" in {
        ...
        val result =
          emailService.sendEmail(subscriptionId, submissionTime, messageRefId, Some(agentSingleContactDetails), isUploadSuccessful = true, reportType)
