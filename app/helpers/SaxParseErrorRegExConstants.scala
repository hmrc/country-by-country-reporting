...
trait SaxParseErrorRegExConstants {
  final val missingDeclarationErrorFormat =
    """cvc-elt.1: Cannot find the declaration of element '(?:cbc:)?(.*?)'.""".stripMargin.r

  final val missingAttributeErrorFormat = """cvc-complex-type.4: Attribute '(.*?)' must appear on element '(?:cbc:)?(.*?)'.""".stripMargin.r

  final val missingOrInvalidErrorFormat = """cvc-type.3.1.3: The value '((?s).*)' of element '(?:cbc:)?(.*?)' is not valid.""".stripMargin.r
  ...
  final val invalidTypeErrorFormat =
    """cvc-attribute.3: The value '((?s).*)' of attribute '(.*?)' on element '(?:cbc:)?(.*?)' is not valid with respect to its type, '(.*?)'.""".stripMargin.r
  ...
  final val genericInvalidSecondErrorFormat =
    """cvc-complex-type.2.2: Element '(?:cbc:)?(.*?)' must have no element children, and the value must be valid.""".stripMargin.r
  ...
  final val unorderedTagErrorFormat =
    """cvc-complex-type.2.4.d: Invalid content was found starting with element '(?:cbc:)?(.*?)'. No child element is expected at this point.""".stripMargin.r
}
